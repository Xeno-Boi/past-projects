...
BackgroundControllerObject::BackgroundControllerObject(Geometry* geom, Shader* shader, GLuint* texture, GameObject* player)
	...
	this->player = player;
	// add first tile
	BackgroundTileObject* tile = new BackgroundTileObject(glm::vec3(0, 0, 0), geom, shader, texture[0], player);
	...
	for (int i = 0; i < tiles_.size(); i++) {
		...
		if (!target_tile->isVisited()) {
			...
			if (distance <= spawn_distance) {		
				...
				// top left tile
				new_position = glm::vec3(target_position.x - tile_scale, target_position.y + tile_scale, 0.0f);
				spawnTile(new_position);
				
				// top tile
				new_position = glm::vec3(target_position.x, target_position.y + tile_scale, 0.0f);
				spawnTile(new_position);
				...
				// right
				new_position = glm::vec3(target_position.x + tile_scale, target_position.y, 0.0f);
				spawnTile(new_position);
...
void BackgroundControllerObject::Render(glm::mat4 view_matrix, double current_time) {
	for (int i = 0; i < tiles_.size(); i++) {
		...
		float distance = glm::length(tiles_[i]->GetPosition() - player->GetPosition());

...
function submit() {
    if (confirm("Are you sure?")) { // user confirms submittion
        let data = {};
        data["cart"] = JSON.parse(JSON.stringify(cart)); // clone cart
        Object.keys(data["cart"]).forEach(id => {
            data["cart"][id]["name"] = restaurant.menu[data["cart"][id]["c"]][id].name; // get item name
        })
        data["subtotal"] = sumCart();
        data["total"] = data["subtotal"] * 1.1 + dFee; // [cart, total, restaurant]
